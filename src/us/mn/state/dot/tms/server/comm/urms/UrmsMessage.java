 ...
 */
public class UrmsMessage implements CommMessage {
	...
	/** Device property */
	protected UrmsProperty dev_prop;

	/** Timer gate, passed via constructor */
	private final TimedGate timer_gate;

	/** Create a new message. */
	public UrmsMessage(OutputStream o, InputStream i, TimedGate tg) {
		UrmsPoller.log("called");
		out_stream = o;
		inp_stream = i;
		timer_gate = tg;
	}
	...
	public void add(ControllerProperty cp) {
		UrmsPoller.log("adding property");
		if(cp instanceof UrmsProperty)
			dev_prop = (UrmsProperty)cp;
	}

	/**
	 * Read data from traffic controler and update controller statuses.
	...
	/**
	 * Read a datagram.
	 * Timeouts (SocketTimeoutException) are silently * caught and handled.
