...
/**
 * Implementation of the EIS Remote Traffic Microwave Sensor (RTMS)
 * protocol using UDP. An object of this class is instantiated once,
 * per defined CommLink using this protocol. This driver supports
 * multiple field devices forwarding UDP packets to the same port.
 * The sensor id field in each VDS' station sample is used to match
 * the drop value specified for each controller in the comm link.
 * The instantiated operation reads from the input stream continuously.
 *
 * @author Michael Darter
 * @author Travis Swanston
 */
public class RtmsPoller extends MessagePoller<RtmsProperty>
	implements SamplePoller
{

	/** Debug log */
	static protected final DebugLog RTMS_LOG = new DebugLog("rtms");
	...
	/** Constructor */
	public RtmsPoller(String n, Messenger m) {
		super(n, m);
	...
	public CommMessage createCommMessageOp(Operation o) throws IOException {
		log("cl=" + comm_link);
		return new RtmsMessage(comm_link,
			messenger.getOutputStream(null),
