...
/**
 * Wizard Property, which reads a traffic sample in a datagram from a field
 ...
 */
public class WizardProperty extends ControllerProperty {
	 ...
	 */
	public WizardProperty(LinkedList<Controller> acs) {
		active_ctrls = acs;
	}
	 ...
	 * parsing into a record and storing. Called by WizardMessage.
		...
		throws IOException
	{
		...
		WizardPoller.log("Returned from read(), " +
			"#bytes=" + bb.size() + ", oaddr=" + oa);
		WizardRec rec = new WizardRec(bb, oa);
	...
	static private InetAddress read(InputStream ais, ByteBlob bb)
		throws IOException
	{
		WizardPoller.log("reading a datagram");
		...
		oa = is.getOriginAddr();
		WizardPoller.log("read datagram=" + bb);
		WizardPoller.log("datagram origin=" + oa);
		return oa;
	}
	...
	public void updateControllers()
	{
		...
		WizardPoller.log("Updating controller states for " +
	...
	private void updateController(ControllerImpl ci, long now) {
		...
		WizardPoller.log("c=" + ci + ", isFailed=" +
		...
		if (ms) {
			...
			WizardRec.storeMissingSample(ci, st);
	...
	static private long nextStorageTime(long lt, long now) {
		...
		WizardPoller.log("now - nt=" + new Long(now - nt));
		if (now - nt > 2 * getReadMarginMs()) {
			WizardPoller.log("delta is larger than 2 x " +
	...
	static protected int getReadMarginMs() {
		return SystemAttrEnum.WIZARD_READ_MARGIN_SEC.getInt() * 1000;
	}
