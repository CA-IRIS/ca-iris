...
public class ByteBlob {
	 ...
	 * @param ns New size. Values less than 0 are assumed to be 0.
	 *           Values greater than the current size expand the buffer.
	 */
	...
	/**
	 * Validate index.
	 * @return The corrected index, or 0 if size() is 0.
	 */
	 ...
	 * @param fi First index, inclusive.
	 * @param ti Terminating index, inclusive.
	 */
	public int crc16() {
		CRC16 crc = new CRC16();
		for(byte b : toArray())
	 ...
	 */
	public int crc16(int si, int ei) {
		CRC16 crc = new CRC16();
		for(byte b : getByteArray(si, ei))
			crc.update(b);
		return (int)crc.getValue();
	}
