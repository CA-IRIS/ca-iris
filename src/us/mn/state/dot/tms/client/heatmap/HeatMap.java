/*
 * Copyright (C) 2007, 2008  Matthew Beckler, Josh Hayes-Sheen and J. Keller
 *
 ...
 */
package us.mn.state.dot.tms.client.heatmap;
...
public class HeatMap extends JPanel {
	 ...
	 */
	public HeatMap(double[][] data, boolean useGraphicsYAxis,
		       Color[] colors) {
		...
		this.setPreferredSize(
			new Dimension(60 + data.length, 60 + data[0].length));
	...
	public void paintComponent(Graphics g) {
		...
		g2d.drawImage(bufferedImage, 31, 31, width - 30, height - 30, 0,
		...
		if (drawTitle && title != null) {
			g2d.drawString(title, (width / 2) - 4 * title.length(),
				       20);
		}
		...
		// Y-Axis ticks
		if (drawYTicks) {
			int yDist = (int) ((height - 60)
					   / (double) numYTicks); //distance between ticks
			for (int y = 0; y <= numYTicks; y++) {
				g2d.drawLine(26, height - 30 - y * yDist, 30,
					     height - 30 - y * yDist);
				label = df.format(((y / (double) numYTicks) * (
					yMax - yMin)) + yMin);
				int labelY = height - 30 - y * yDist - 4 * label
					.length();
		...
		if (drawYTitle && yAxis != null) {
			//to get the text to fit nicely, we need to rotate the graphics
			g2d.rotate(Math.PI / 2);
			g2d.drawString(yAxis, (height / 2) - 4 * yAxis.length(),
				       -3);
			g2d.rotate(-Math.PI / 2);
		}
		...
		// X-Axis ticks
		if (drawXTicks) {
			int xDist = (int) ((width - 60)
					   / (double) numXTicks); //distance between ticks
			for (int x = 0; x <= numXTicks; x++) {
				g2d.drawLine(30 + x * xDist, height - 30,
					     30 + x * xDist, height - 26);
				label = df.format(((x / (double) numXTicks) * (
					xMax - xMin)) + xMin);
				int labelX =
					(31 + x * xDist) - 4 * label.length();
		...
		if (drawXTitle && xAxis != null) {
			g2d.drawString(xAxis, (width / 2) - 4 * xAxis.length(),
				       height - 3);
		}
		...
		if (drawLegend) {
			...
			for (int y = 0; y < height - 61; y++) {
				int yStart = height - 31 - (int) Math
					.ceil(y * ((height - 60) / (
						colors.length * 1.0)));
				yStart = height - 31 - y;
				g2d.setColor(colors[(int) (
					(y / (double) (height - 60)) * (
						colors.length * 1.0))]);
